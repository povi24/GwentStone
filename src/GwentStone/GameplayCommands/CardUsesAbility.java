...
public class CardUsesAbility {
    ...
    public CardUsesAbility(ActionsInput action, StartingTheGame newgame, ObjectMapper objectMapper) {
        ...
        if (newgame.getCurrentTurn() == 1) {
            ...
            if ((cardAttacked.getX() == 2 || cardAttacked.getX() == 3)
                    && (newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY()).getName().equals("Miraj")
                    || newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY()).getName().equals("The Ripper")
                    || newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY()).equals("The Cursed One"))) {
                showError = true;
            ...
            } else if ((cardAttacked.getX() == 0 || cardAttacked.getX() == 1)
            && newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY()).getName().equals("Disciple")) {
                showError = true;
                this.error = "Attacked card does not belong to the current player.";
                node.put("command", command);
                node.putPOJO("cardAttacker", cardAttacker);
                node.putPOJO("cardAttacked", cardAttacked);
                node.put("error", error);
                ...
                newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY())
                        .minionAbility(newgame.getTable().getTable(), cardAttacked.getX(), cardAttacked.getY());
            ...
            if ((cardAttacked.getX() == 0 || cardAttacked.getX() == 1)
                ...
                node.putPOJO("cardAttacked", cardAttacked);
                node.put("error", error);
            } else if ((cardAttacked.getX() == 2 || cardAttacked.getX() == 3)
                    && newgame.getTable().getTable().get(cardAttacker.getX()).get(cardAttacker.getY()).getName().equals("Disciple")) {
                showError = true;
                this.error = "Attacked card does not belong to the current player.";
                node.put("command", command);
